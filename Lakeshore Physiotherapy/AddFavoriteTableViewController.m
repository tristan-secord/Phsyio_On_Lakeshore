...
#include "FavoritesTableViewController.h"
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {
        NSLog(@"file exists");
        favsArray = [[NSMutableArray alloc]initWithContentsOfFile:path];
    } else {
    ...
    sectionArray = [dic valueForKey:keys[indexPath.section]];
    
    if (self.searchDisplayController.active) {
        _selectedEx = [searchResults objectAtIndex:indexPath.row];
    } else {
        _selectedEx = [sectionArray objectAtIndex:indexPath.row];
    }
    
    [favsArray insertObject:_selectedEx atIndex:0];
    
    [favsArray writeToFile:path atomically:YES];
    [self dismissViewControllerAnimated:YES completion:nil];
}
